...
class FocuserController:
    ...
    def goZero(self):
        ...
        if self.ser and self.ser.is_open:
            ...
            time.sleep(30)  # Adjust this sleep duration if necessary
            self.ser.write(b'S')
    ...
    def getTime(self, position):
        ...
        for i in fsolve(f, [-20,20]):
            if i > 0:
                self.new_time = i
        print(f"D = {position} mm, Time = {self.new_time} s")

    def getPosition(self, _time):
        self.position = _time * self.speed
